var gameZkpState = new Phaser.Class({
    ...
    create: function() {
        ...
        zkpWin = this.sound.add('zkp-win', { loop: false });
        ...
        out.setInteractive().on('pointerdown', function() { 
            BGM_ZKP.stop(); BGM_MAP.resume();
            game.scene.stop('zkp'); game.scene.wake('2000'); 
        }, this);
    ...
    update: function() {
        ...
        if (this.isGameOver()) {
            game.scene.stop('zkp');
            game.scene.wake('2000');
        } else if (this.isWon()) {
            BGM_ZKP.stop(); BGM_MAP.resume(); EFFECT_CLEAR.play();
            Collection.push('zkp');
            game.scene.stop('zkp');
            game.scene.wake('2000');
        } else {
