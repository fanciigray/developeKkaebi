var gameJoyState = new Phaser.Class({
    ...
    create: function() {
        ...
        joyCollect = this.sound.add('joy-collect', { loop: false });
        ...
        out.setInteractive().on('pointerdown', function() { 
            BGM_JOY.stop(); BGM_MAP.resume();
            game.scene.stop('joy'); game.scene.wake('7000'); 
        }, this)
        ...
        this.timer = this.add.text(
            870, 198, this.initialTime.toString(), 
            {fontFamily: 'joyfulStory', fontSize: 36, color: '#000000'}
        ).setDepth(100);
    ...
    update: function() {
        ...
        if (this.isGameOver()) {
            BGM_JOY.stop(); BGM_MAP.resume(); EFFECT_GAMEOVER.play();
            game.scene.stop('joy');
            game.scene.wake('7000');
        } else if (this.isWon()) {
            BGM_JOY.stop(); BGM_MAP.resume(); EFFECT_CLEAR.play();
            Collection.push('joy');
            game.scene.stop('joy');
            game.scene.wake('7000');
        } else {
