var gameCiderState = new Phaser.Class({
    ...
    create: function() {
        ...
        ciderPop = this.sound.add('cider-pop', { loop: false });
        ...
        out.setInteractive().on('pointerdown', function() { 
            BGM_MAP.resume(); BGM_CIDER.stop();
            game.scene.stop('cider'); game.scene.wake('8000'); 
        }, this)
        ...
        this.dust1.children.iterate(function(child) {
            child.setInteractive().on('pointerdown', function() {
                if (!pressToStart.visible) { child.disableBody(true, true); ciderPop.play(); }
            }, this)
        })
    ...
    update: function() {
        ...
        if (this.isGameOver()) {
            BGM_MAP.resume(); BGM_CIDER.stop(); EFFECT_GAMEOVER.play();
            game.scene.stop('cider');
            game.scene.wake('8000');
        } else if (this.isWon()) {
            BGM_MAP.resume(); BGM_CIDER.stop(); EFFECT_CLEAR.play();
