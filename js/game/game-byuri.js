var gameByuriState = new Phaser.Class({
    ...
    create: function() {
        ...
        this.correct = this.sound.add('byuri-correct', { loop: false });
        this.no = this.sound.add('byuri-no', { loop: false });
        ...
        out.setInteractive().on('pointerdown', function() {
            BGM_BYURI.stop(); BGM_MAP.resume();
            game.scene.stop('byuri'); game.scene.wake('4000');  
        }, this);
        ...
        this.answer1 = this.add.sprite(675, 510, 'byuri-correct', 0).setInteractive().on('pointerdown', function() { 
            ...
            this.foundNum += 1; this.correct.play();
        }, this);
    ...
    update: function() {
        ...
        if (this.isGameOver()) {
            BGM_BYURI.stop(); BGM_MAP.resume(); EFFECT_GAMEOVER.play();
            game.scene.stop('byuri');
            game.scene.wake('4000');
        } else if (this.isWon()) {
            BGM_BYURI.stop(); BGM_MAP.resume(); EFFECT_CLEAR.play();
